  ...
  const executiveCockpit = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard title="Bed Occupancy" value={kpiVal('Bed Occupancy', '84.2%')} status="warning" />
        <KPICard title="ICU Availability" value={kpiVal('ICU Availability', '12 beds')} status="danger" />
        <KPICard title="Avg Length of Stay" value={kpiVal('Avg Length of Stay', '4.2 days')} status="neutral" />
        <KPICard title="Facilities Monitored" value={kpiVal('Facilities Monitored', '124')} status="neutral" />
      </div>
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="lg:col-span-1">
          <GeoMap
            country="thailand"
            labels={{ entity: 'Hospitals', event: 'Bed Census', alert: 'Capacity Alerts' }}
      ...
      <DataTable
        columns={[
          ...
          { key: 'm1', header: 'Occupancy %' },
          { key: 'm2', header: 'Icu Availability' },
          { key: 'm3', header: 'Length of Stay' },
          { key: 'events', header: 'Bed Census' },
          { key: 'alerts', header: 'Capacity Alerts' },
        ]}
        data={data?.entities || []}
  ...
  const domainTab1 = (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <KPICard title="Predicted Surge (7d)" value={kpiVal('Predicted Surge (7d)', '+8%')} />
        <KPICard title="Surgery Queue" value={kpiVal('Surgery Queue', '342')} />
        <KPICard title="ER Wait (Avg)" value={kpiVal('ER Wait (Avg)', '47 min')} />
      </div>
      <Chart
